...
The `0.3.0` assembler accepts `0.1.0`, `0.2.0`, and `0.3.0` fragments and emits only `0.3.0`. Consumers must reject unsupported major versions rather than guessing. Preserve unknown `data` fields, but validate decision-bearing fields such as disposition, verification, severity, and layer.
