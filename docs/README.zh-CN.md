...
- 从 PBX source/resource phase 和可选 `xcodebuild` metadata 建立稳定 target graph。
...
- 检查 parent/child ID、版本、平台、架构、embedded profile、调试资源、static library 和 Xcode Privacy Report 证据。
...
- 只读导入本地 App Store Connect export，并与 Archive 的 build identity 对比。
